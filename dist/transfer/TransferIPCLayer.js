...
class TransferIPCLayer extends events_1.EventEmitter {
    ...
    send(messageToSend) {
        if (!this.worker) {
            return Promise.reject(new Errors_1.MessageTransferRejected(`Call was rejected, worker is not exists anymore.`, messageToSend.stackTrace));
        }
        return new Promise((resolve, reject) => {
            ...
            const messageHandler = (message) => {
                if (typeof message === 'object' && !Array.isArray(message) &&
                    ...
                    if (message.error) {
                        reject(new Errors_1.MessageResultError(message.error.message, message.error.stack, message.error.originalStack));
                    }
                    else {
            ...
            const rejectHandler = (message) => {
                this.removeListener(exports.EVENT_WORKER_CHANGED, rejectHandler);
                this.worker.removeListener('message', messageHandler);
                reject(new Errors_1.MessageTransferRejected(`Call was rejected, due to worker was chenged.`, messageToSend.stackTrace));
            };
